...
export default function (config, helper) {
  ...
  const Bars = Object.create(helper);
  ...
  Bars.data = function (data) {
    ...
    if (vm._config.sortBy) {
      vm._data = vm._data.sort((a, b) => a[vm._config.sortBy[0]] - b[vm._config.sortBy[0]]);
    }

    if (vm._config.hasOwnProperty('quantiles')) {
      vm._quantiles = vm._setQuasecondntile(data);
      vm._minMax = d3.extent(data, d => +d[vm._config.fill]);
    }
  ...
  Bars.drawLabels = function () {
    ...
    const charContainer = vm.chart.svg().selectAll('.dbox-label')
      .data(vm._data);

    charContainer.enter().append('text')
      .attr('class', 'dbox-label')
      .attr('x', (d) => {
        let value = vm._scales.x(d[vm._config.x]);
      ...
      .attr('y', (d) => {
        let value = vm._scales.y(d[vm._config.y]);
        const barH = vm._scales.y.bandwidth ? vm._scales.y.bandwidth() : Math.abs(vm._scales.y(d[vm._config.y]) - vm._scales.y(0));
      ...
      .attr('transform', (d) => {
        const barW = vm._scales.x.bandwidth ? vm._scales.x.bandwidth() : Math.abs(vm._scales.x(d[vm._config.x]) - vm._scales.x(0));
  ...
  Bars.draw = function () {
    ...
    vm.chart.svg().selectAll('.bar')
      ...
      .attr('width', d => (vm._scales.x.bandwidth ? vm._scales.x.bandwidth() : Math.abs(vm._scales.x(d[vm._config.x]) - vm._scales.x(0))))
      .attr('height', d => (vm._scales.y.bandwidth ? vm._scales.y.bandwidth() : Math.abs(vm._scales.y(d[vm._config.y]) - vm._scales.y(0))))
      .attr('fill', (d) => {
      ...
      .on('mouseover', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) { // OnHover colors
          d3.select(this).attr('fill', d => vm._getQuantileColor(d[vm._config.fill], 'onHover'));
        }
      ...
      .on('mouseout', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) { // OnHover reset default color
          d3.select(this).attr('fill', d => vm._getQuantileColor(d[vm._config.fill], 'default'));
        }
        vm._tip.hide();
  ...
  Bars.drawGroupLabels = function () {
    ...
    vm.chart.svg().selectAll('.grouped').each(function () {
      ...
      d3.select(this).selectAll('rect').each((dat, index) => {
        d3.select(el).append('text')
          ...
          .attr('text-anchor', 'middle')
          .text(d => (d[vm._config.groupBy[index]] ? vm.utils.format(d[vm._config.groupBy[index]], true, vm._config.decimals) : ''));
        ...
        d3.select(el).append('text')
          ...
          .attr('text-anchor', 'middle')
          .text(d => (d[`${vm._config.groupBy[index] }coefficient`] && !Number.isNaN(d[`${vm._config.groupBy[index] }coefficient`]) ? `(${ d[`${vm._config.groupBy[index]  }coefficient`].toFixed(1)})` : ''));
      });
    });
  };
  ...
  Bars._drawGroupByXAxis = function () {
    ...
    vm.chart.svg().append('g')
      ...
      .append('g')
      .attr('class', 'grouped')
      .attr('transform', d => `translate(${vm._scales.x(d[vm._config.x])},0)`)
      .selectAll('rect')
      .data(d => vm._config.groupBy.map(key => ({
        key,
        value: d[key],
        axis: d[vm._config.x],
      })))
      .enter()
      .append('rect')
      .attr('x', d => vm._scales.groupBy(d.key))
      .attr('y', (d) => {
        if (d.value > 0) {
      ...
      .attr('height', d => Math.abs(vm._scales.y(d.value) - vm._scales.y(0)))
      .attr('fill', d => vm._scales.color(d.key))
      .on('mouseover', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) { // OnHover colors
          d3.select(this).attr('fill', d => vm._getQuantileColor(d[vm._config.fill], 'onHover'));
        }
  ...
  Bars._drawGroupByYAxis = function () {
    ...
    vm.chart.svg().append('g')
      ...
      .append('g')
      .attr('class', 'grouped')
      .attr('transform', d => `translate(0,${vm._scales.y(d[vm._config.y])} )`)
      .selectAll('rect')
      .data(d => vm._config.groupBy.map(key => ({
        key,
        value: d[key],
      ...
      .enter()
      .append('rect')
      .attr('y', d => vm._scales.groupBy(d.key))
      .attr('x', (d) => {
        if (d < 0) {
      ...
      .attr('width', d => Math.abs(vm._scales.x(d.value) - vm._scales.x(0)))
  ...
  Bars.drawStackLabels = function () {
    const vm = this;
    const groups = vm.chart.svg().selectAll('.division');
    let index = 0;
    groups.each(function (data) {
      const stacks = d3.select(this).selectAll('rect').nodes();
      // Get rect height to condition label drawing
      data.forEach((d) => {
        const el = this;
        const dat = d;
        const rect = stacks[index];
        index++;
        const rectH = rect.getBBox().height;
        ...
        if (rectH > 35) {
          d3.select(el).append('text')
            ...
            .text(() => (dat.data[data.key] ? vm.utils.format(dat.data[data.key], true, vm._config.decimals) : ''));
        }
      });
      index = 0;
    });
  ...
  Bars._drawStackByXAxis = function () {
    ...
    vm.chart.svg().append('g')
      ...
      .enter()
      .append('rect')
      .attr('y', d => vm._scales.y(d[1]))
      .attr('x', d => vm._scales.x(d.data[vm._config.x]))
      .attr('width', () => vm._scales.x.bandwidth())
      .attr('height', d => vm._scales.y(d[0]) - vm._scales.y(d[1]))
      .on('mouseover', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) { // OnHover colors
          d3.select(this).attr('fill', d => vm._getQuantileColor(d[vm._config.fill], 'onHover'));
        }
  ...
  Bars._drawStackByYAxis = function () {
    ...
    vm.chart.svg().append('g')
      ...
      .enter()
      .append('rect')
      .attr('y', d => vm._scales.y(d.data[vm._config.y]))
      .attr('x', d => vm._scales.x(d[0]))
      .attr('height', () => vm._scales.y.bandwidth())
      .attr('width', d => vm._scales.x(d[1]) - vm._scales.x(d[0]))
      .on('mouseover', function (d, i) {
        if (vm._config.hasOwnProperty('quantiles') && vm._config.quantiles.hasOwnProperty('colorsOnHover')) { // OnHover colors
          d3.select(this).attr('fill', d => vm._getQuantileColor(d[vm._config.fill], 'onHover'));
        }
    ...
    if (vm._config && vm._config.bars.quantiles && vm._config.bars.quantiles.colors) {
      if (vm._quantiles.length > 2) {
        ...
        if (type == 'default') {
          ...
          } if (total <= vm._quantiles[2]) {
            return vm._config.bars.quantiles.colors[1]; //'#e65158';
          } if (total <= vm._quantiles[3]) {
